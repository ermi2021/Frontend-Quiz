...
export default {
  ...
  setup() {
    ...
    const fetchQuestionsFromApi = async function (api) {
      fetch(api)
        .then((res) => {
          return res.json();
        })
    ...
    const onQuizStart = function () {
     ...
     if($route.params.catagory == 'film') {
        fetchQuestionsFromApi('https://opentdb.com/api.php?amount=10&category=11&type=multiple');
      }else{
        fetchQuestionsFromApi('https://opentdb.com/api.php?amount=10&category=18&type=multiple');
      }
    ...
    onMounted(() => {
     ...
     const catagoryApi = api.filter((api)=>{
        return api.catagory == catagory;
      });
      console.log(catagoryApi);
      fetchQuestionsFromApi('https://opentdb.com/api.php?amount=10&category=20')
    });
